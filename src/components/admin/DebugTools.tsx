...
export function DebugTools() {
  const { showDebugTools, setShowDebugTools, isAdminMode } = useEditor();
  const [enabled, setEnabled] = useState<PlaygroundDebugLayer[]>(["hard-safe", "slot-labels"]);

  if (!isAdminMode || !showDebugTools) return null;
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <p className="mb-4 text-sm text-muted-foreground">
          Toggle debug overlays for preview and playground sessions. Open Motion Block Playground
          for live preview with these layers.
        </p>
