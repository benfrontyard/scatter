...
import { TransitionDebugPanel } from "@/components/studio/TransitionDebugPanel";
...
export function StudioDiagnostics() {
  ...
  const registryHealth = useMemo(() => {
    ...
    const published = getBlocksByStatus("approved").concat(
      getBlocksByStatus("published" as typeof blocks[0]["status"]),
    );
    const visibilityIssues = published.flatMap((b) => getApprovedBlockVisibilityIssues(b));
    const notInLibrary = published.filter((b) => !isApprovedBlockUserLibraryReady(b));
    return { total: blocks.length, published: published.length, visibilityIssues, notInLibrary };
  }, []);
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Registry health, renderer coverage, validation tools, and dev utilities. Not part of the
        ...
        <section className="rounded-lg border border-border p-4">
          ...
          <ul className="space-y-1 text-xs text-muted-foreground">
            ...
            <li>Block Workbench prefers production renderer when editorBlockId is bridged</li>
          </ul>
        </section>
        ...
        <section className="rounded-lg border border-border p-4">
          <div className="mb-3 flex items-center gap-2">
            <Activity className="h-4 w-4 text-muted-foreground" />
            <h3 className="text-sm font-semibold">Transition debug</h3>
          </div>
          <TransitionDebugReadout />
        </section>
        ...
        <section className="rounded-lg border border-border p-4">
          ...
          <dl className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
            ...
            <div>
              ...
              <dd className="text-lg font-semibold">{registryHealth.published}</dd>
            </div>
            <div>
...
function TransitionDebugReadout() {
  const { sequence, selectedBlockId } = useEditor();
  const blockIndex = selectedBlockId
    ? sequence.blocks.findIndex((b) => b.id === selectedBlockId)
    : 0;

  if (sequence.blocks.length === 0) {
    return <p className="text-xs text-muted-foreground">No blocks in sequence.</p>;
  }

  return <TransitionDebugPanel blockIndex={Math.max(0, blockIndex)} />;
}

/** @deprecated Use StudioDiagnostics */
export const StudioSystem = StudioDiagnostics;
